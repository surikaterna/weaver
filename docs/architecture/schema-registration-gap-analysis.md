...
- Public configuration paths under `/_weaver` are reserved for Weaver metadata and are hidden from normal reads and protected from normal writes.
- The persistent schema registry stores its internal document at `_weaver.registry.schemas` through private read/write access so restart hydration does not expose that document through public configuration APIs.
