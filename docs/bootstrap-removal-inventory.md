...
Server test orchestration uses an exact manifest of 24 Turbo partitions. The repository-root `pnpm run test` is the canonical non-forced gate and runs Turbo at concurrency 8; each partition remains internally serial. Jenkins builds first in the same workspace, so the test gate reuses the successful server build rather than forcing another DTS build. Every discovered Node and Vitest file must be assigned exactly once, and manifest verification rejects missing, stale, duplicate, wrong-suite, unknown, or empty assignments. A named partition can be run directly with `pnpm --filter @weaver-conf/weaver-server run test:<partition>`. Source partitions depend on dependency-package builds, while only the CLI partition waits for the server build. This split does not skip tests, increase timeouts, or delete coverage.
