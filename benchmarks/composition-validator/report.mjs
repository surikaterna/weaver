...
function serverSection(raw) {
  const rows = raw.gates.server.map((item) => [item.id, fixed(item.p50), fixed(item.ordinaryP50), fixed(item.ratio), item.gated ? ratioVerdict(item.ratio, raw.thresholds.server) : "characterized"]);
  ...
  return `## Server full-candidate gates\n\nWorst gated two-branch ratio: **${worst.id} ${fixed(worst.ratio)}x** (limit 2.0). not/shared-40 are safety-characterized only.\n\n${table(["case", "p50", "ordinary p50", "ratio", "gate"], rows)}`;
}
...
function render(raw, rawText) {
  ...
  const sections = [`# Composition validator final benchmark report\n\n## Verdict: ${raw.verdict}\n\n${reasonText}`, evidenceSection(raw, rawHash), methodSection(raw)];
