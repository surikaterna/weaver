...
function addOrdinary(descriptors) {
  ...
  addOrdinaryPatchAndServer(descriptors);
}

function addOrdinaryPatchAndServer(descriptors) {
  for (const expectedValid of [true, false]) {
    const suffix = expectedValid ? "valid" : "invalid";
    descriptors.push({ id: `ordinary:patch:${suffix}`, family: "ordinary", variants: "both", kind: "engine", method: "patch", schema: { type: "object", properties: { value: { type: "string" } }, additionalProperties: false }, path: "value", value: expectedValid ? "ok" : 1, expectedValid });
    descriptors.push({ id: `ordinary:server:${suffix}`, family: "server-ordinary", variants: "both", kind: "server", serverCase: "ordinary", expectedValid });
  }
}

function branchShape(keyword, count, position) {
  ...
  for (const keyword of ["anyOf", "oneOf"]) {
    for (const count of [2, 8, 32, 128]) {
      for (const position of ["first", "last", "none", "multiple"]) {
        const [schema, value, expectedValid] = branchShape(keyword, count, position);
        descriptors.push({ id: `${keyword}:${count}:${position}`, family: `${keyword}-${position}`, variants: "tip", kind: "engine", method: "partial", schema, value, expectedValid, work: count, position });
      }
    }
  }
}
...
function sharedDiamond(depth, leaf) {
  ...
  for (let index = 0; index < depth; index += 1) {
    const openObject = leaf.type === "object" ? { additionalProperties: true } : {};
    schema = { type: leaf.type, allOf: [schema, schema], ...openObject };
  }
  return schema;
}
...
function distinctAllOf(count, mode) {
  ...
  if (mode === "first-failure") branches[0] = { type: "number", minimum: 51 };
  if (mode === "last-failure") branches[count - 1] = { type: "number", maximum: 49 };
  return [{ type: "number", allOf: branches }, 50, mode === "all-match"];
}
...
function addMixedAndScale(descriptors) {
  for (const depth of [8, 32, 128]) {
    for (const valid of [true, false]) descriptors.push({ id: `mixed:${depth}:${valid ? "valid" : "invalid"}`, family: `mixed-${valid ? "valid" : "invalid"}`, variants: "tip", kind: "engine", method: "partial", schema: mixedSchema(depth), value: valid ? "ok" : "bad", expectedValid: valid, work: depth });
  }
  for (const valid of [true, false]) descriptors.push({ id: `not:${valid ? "mismatch" : "match"}`, family: "not", variants: "tip", kind: "engine", method: "partial", schema: { type: "string", not: { type: "string", const: "blocked" } }, value: valid ? "ok" : "blocked", expectedValid: valid });
  addLinearAndScale(descriptors);
}

function addLinearAndScale(descriptors) {
  for (const depth of [100, 1000, 5000]) {
    for (const valid of [true, false]) {
      const [schema, value] = linearShape(depth, valid);
      descriptors.push({ id: `linear:${depth}:${valid ? "valid" : "invalid"}`, family: `linear-${valid ? "valid" : "invalid"}`, variants: "tip", kind: "engine", method: "partial", schema, value, expectedValid: valid, work: depth });
    }
  }
...
function addPatches(descriptors) {
  ...
  for (const valid of [true, false]) descriptors.push({ id: `patch:leaf:${valid ? "valid" : "invalid"}`, family: "patch-leaf", variants: "tip", kind: "engine", method: "patch", schema: { type: "object", properties: { leaf: { type: "string", anyOf: [{ type: "string", const: "ok" }] } }, additionalProperties: false }, path: "leaf", value: valid ? "ok" : "bad", expectedValid: valid });
}

function addServer(descriptors) {
  const cases = [["anyOf-valid", true], ["anyOf-invalid", false], ["oneOf-ambiguity", false], ["allOf-valid", true], ["allOf-invalid", false], ["not-rejection", false], ["shared-40", false]];
  for (const [serverCase, expectedValid] of cases) descriptors.push({ id: `server:${serverCase}`, family: "server-composition", variants: "tip", kind: "server", serverCase, expectedValid });
}
...
export async function buildFixture(descriptor, api) {
  if (descriptor.kind === "engine") {
    const operation = descriptor.method === "partial"
      ? () => api.validatePartialConfiguration(descriptor.schema, descriptor.value)
      : descriptor.method === "effective"
        ? () => api.validateEffectiveConfiguration(descriptor.schema, descriptor.value)
        : () => api.validateConfigurationPatch(descriptor.schema, descriptor.path, descriptor.value);
    return { async: false, roots: [descriptor.schema, descriptor.value], operation, effects: () => ({}) };
  }
  return buildServerFixture(descriptor, api);
}

async function buildServerFixture(descriptor, api) {
...
export function fingerprint(roots) {
  ...
  while (stack.length > 0) {
    ...
    if (value === null || typeof value !== "object") {
      hash.update(`${path}:${typeof value}:${String(value)};`);
      continue;
    }
    if (seen.has(value)) {
      hash.update(`${path}:ref:${seen.get(value)};`);
      continue;
    }
    ...
    for (let index = keys.length - 1; index >= 0; index -= 1) {
      ...
      hash.update(`${String(key)}:${property.enumerable}:${property.configurable}:${"writable" in property ? property.writable : "accessor"};`);
      if ("value" in property) stack.push([property.value, `${path}.${String(key)}`]);
    }
  }
